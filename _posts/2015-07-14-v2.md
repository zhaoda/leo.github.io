...
You don't know about it? No problem! It's basically a new, open-source font which is now mainly used it Firefox OS. It's available in over 16 weights and I completely fell in love with every single one! What I especially like about it is that the letter **l** has such a nice curve[^3] at the bottom, which flows like the rounded bottom of the letter **e**.

{% include image.liquid file="name-fira.png" %}
...
Since I released my first blog posts a few years, I always stood glued to RSS for providing a feed. Mostly because I didn't know of any other alternative and maybe also because the themes I've used didn't have the Atom feed url in their `head` tag yet (therefore I never really got to experiment with it).
...
Things like that are important to me, because they make me feel that the people who thought out this format spent much of their time just to make it simpler and avoid confusion. And that kind of effort is always one I will value.
...
- The `id` key doesn't need a `isPermaLink` attribute anymore, if it contains one.
...
The last theme I used on this blog wasn't really optimized for this. It felt like it was created before HTML 5 and hasn't been updated since then. Since I am a web developer and very focused on creating semantically correct sites which can be easily interpreted by every software/device, I needed to change this.

Starting with the way my articles are displayed, I've created a completely new theme which doesn't completely rely on divs anymore and I'm very happy with the result.
...
... and I banged my head on the table, yelled "FUCK THE WOOORLD!!" and changed my post's permalink root from **/blog** to **/notes**.

## What I learned from this

Before I finish this article (probably one of the longest I've ever written), I want to leave a few words about how I actually came to this point where I figured out how my new site will look and work like:
...
But I think I found out was my problem was: I wanted to create in one flow, without a pause. But now I know, that this is exactly what design needs: Time. Because I tried so hard to make something create, the result wasn't quite fitting my expectations. Instead of doing that again, I just did a few things and then left the design some "time to breathe". Meanwhile I had enough time for myself to think about which change I wanted to apply next. And hell yeah — it worked!
